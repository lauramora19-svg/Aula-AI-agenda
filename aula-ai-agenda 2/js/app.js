...
class AppController {
    ...
    window.store.state.slots.forEach(slot => {
      if (slot.isBreak) {
        ...
        window.DAYS.forEach(day => {
          ...
          const subjectId = window.store.getScheduleSubject(day.id, slot.id);
              ...
              <td data-target-key="${cellKey}">
                <div class="class-card ${subj.colorClass}" data-cell-id="${cellKey}" draggable="true" onclick="window.app.openExpandedLessonModal('${cellKey}')">
                  <div class="card-header">
                    ...
                    <span class="group-badge">${(lesson && lesson.group) || 'Sin grupo'}</span>
                  </div>
  ...
  saveLessonFromModal() {
    ...
    // La asignatura se guarda en el horario FIJO (igual todas las semanas).
    window.store.setScheduleSubject(day, slot, subjectId);
    // El resto (qué se va a dar, notas, estado...) se guarda solo para ESTA semana.
    window.store.updateLesson(day, slot, {
      group, title, notes, targetNEAE, status
    });
