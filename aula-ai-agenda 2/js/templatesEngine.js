  ...
  renderSubstitutionDay(dateStr) {
    ...
    window.store.state.slots.forEach(slot => {
      if (slot.isBreak) {
        ...
        const entry = window.store.getScheduleEntry(dayId, slot.id);
        if (entry) {
          const subj = window.DEFAULT_SUBJECTS[entry.subjectId] || {};
          const les = weekLessons[key] || {};
          dayLessons.push({
            time: slot.time,
            label: `${slot.label} - ${subj.name || 'Asignatura'}${entry.group ? ` (${entry.group})` : ''}`,
            title: les.title || 'Sin planificar esta semana',
