...
class AppStore {
  ...
  async loadFromCloud(userId) {
    ...
    try {
      ...
      // Migración: si ya hab�as programado clases antes de separar el horario
      // fijo del contenido semanal, recuperamos la asignatura de cada casilla
      // a partir de lo que ya tuvieras escrito (sin perder nada).
      Object.values(this.state.weeklyLessons).forEach(weekLessons => {
        Object.entries(weekLessons).forEach(([key, lesson]) => {
          if (lesson && lesson.subjectId && !this.state.schedule[key]) {
            this.state.schedule[key] = lesson.subjectId;
          }
        });
      });
    } catch (e) {
  ...
  // Asignatura fija de una casilla (igual todas las semanas del curso)
  getScheduleSubject(dayId, slotId) {
    return this.state.schedule[`${dayId}_${slotId}`] || null;
  }

  setScheduleSubject(dayId, slotId, subjectId) {
    const key = `${dayId}_${slotId}`;
    if (subjectId) {
      this.state.schedule[key] = subjectId;
    } else {
