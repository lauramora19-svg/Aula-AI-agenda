...
class AppController {
    ...
    window.store.state.slots.forEach(slot => {
      if (slot.isBreak) {
        ...
        window.DAYS.forEach(day => {
          ...
          const entry = window.store.getScheduleEntry(day.id, slot.id);
          const subjectId = entry ? entry.subjectId : null;
          const subj = subjectId ? window.DEFAULT_SUBJECTS[subjectId] : null;
          const lesson = window.store.getLesson(day.id, slot.id); // contenido de ESTA semana (puede no existir aún)

          if (subj) {
            let statusClass = 'status-pending';
            let statusText = 'Pendiente';
            const status = lesson ? lesson.status : 'pending';
            if (status === 'done') { statusClass = 'status-done'; statusText = 'Impartida'; }
            if (status === 'shifted') { statusClass = 'status-shifted'; statusText = 'Reprogramada'; }
              ...
              <td data-target-key="${cellKey}">
                <div class="class-card ${subj.colorClass}" data-cell-id="${cellKey}" draggable="true" onclick="window.app.openExpandedLessonModal('${cellKey}')">
                  <div class="card-header">
                    ...
                    <span class="group-badge">${entry.group || 'Sin grupo'}</span>
                  </div>

                  <div class="card-body">
                    <div class="lesson-title">${(lesson && lesson.title) || 'Sin planificar esta semana'}</div>
                    <div class="lesson-notes-snippet">${(lesson && lesson.notes) || ''}</div>
                  </div>
  ...
  openExpandedLessonModal(cellKey) {
    ...
    const entry = window.store.getScheduleEntry(day, slot);
    const lesson = window.store.getWeekLessons()[cellKey] || {
      title: '', notes: '', targetNEAE: '', status: 'pending'
    };

    const subj = entry ? window.DEFAULT_SUBJECTS[entry.subjectId] : null;

    document.getElementById('modal-lesson-title-display').innerText = subj ? `Detalle de Clase — ${subj.name}` : 'Nueva Casilla del Horario';
    document.getElementById('modal-subject-select').value = (entry && entry.subjectId) || 'MATES';
    document.getElementById('modal-group-input').value = (entry && entry.group) || '';
